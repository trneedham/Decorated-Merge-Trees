...
def get_ultramatrix(G,height,label):

    inverted_label = invert_label_dict(label)

    matrix_size = len(list(inverted_label.keys()))

    M = np.zeros([matrix_size,matrix_size])

    for i in range(matrix_size):
        ind1 = inverted_label[i]
        for j in range(matrix_size):
            ind2 = inverted_label[j]
            M[i,j] = least_common_ancestor(G,height,ind1,ind2)[1]
...
def subdivide_and_compute_distance(T1,height1,T2,height2,mesh):
    ...
    res = interleaving_subdivided_trees(T1_sub,height1_sub,T2_sub,height2_sub)

    return T1_sub, height1_sub, T2_sub, height2_sub, res
...
def fusedGW_interleaving_decorated_trees(T1_sub,height1_sub,node_barcode1,
                                        ...
                                        thresh = 1.0, alpha = 1/2, armijo = True, degree_weight = True):
    ...
    for leaf_node in leaf_nodes2:
        ...
        for matched_node in matched_node_row_support:
            ...
            if np.argmax(coup[:,matched_node]) == matched_node_coup_idx:
                ...
                Pi.append((idx_dict1[matched_node_coup_idx],idx_dict2[matched_node]))
    ...
    MNew = get_barcode_matching_matrix(node_barcode1,node_barcode2,labels1NewInverted,labels2NewInverted)
    # For the persistence distance, we only consider bottleneck distances between matched points
    distDgm = np.max([MNew[i,i] for i in range(MNew.shape[0])])
    ...
    res['distDgm'] = distDgm
    res['dist'] = max([distMerge,distDgm])

    return res

def fusedGW_subdivide_and_compute_decorated_distance(T1,height1,barcodes1,
    ...
    res = fusedGW_interleaving_decorated_trees(T1_sub,height1_sub,node_barcode1,
                                        T2_sub,height2_sub,node_barcode2,
                                        thresh = thresh, alpha = alpha, armijo = armijo, degree_weight = degree_weight)
